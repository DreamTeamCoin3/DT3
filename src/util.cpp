...
const char * const BITCOIN_CONF_FILENAME = "dreamteam3.conf";
const char * const BITCOIN_PID_FILENAME = "dreamteam3d.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                if(ptrCategory->count(std::string("dreamteam3"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DreamTeam3
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DreamTeam3
    // Mac: ~/Library/Application Support/DreamTeam3
    // Unix: ~/.dreamteam3
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DreamTeam3";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DreamTeam3";
#else
    // Unix
    return pathRet / ".dreamteam3";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty dreamteam3.conf if it does not excist
