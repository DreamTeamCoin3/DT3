...
*Setup instructions for a Gitian build of DreamTeam3 Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DreamTeam3
...
- [Building DreamTeam3 Core](#building-dreamteam3-core)
...
Clone the git repositories for DreamTeam3 Core and Gitian.
...
in the DreamTeam3 Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building DreamTeam3 Core
----------------

To build DreamTeam3 Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the DreamTeam3 Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dreamteam3/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/dreamteam3pay/dreamteam3
...
For example:
```bash
URL=https://github.com/crowning-/dreamteam3.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit dreamteam3=${COMMIT} --url dreamteam3=${URL} ../dreamteam3/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dreamteam3=${COMMIT} --url dreamteam3=${URL} ../dreamteam3/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dreamteam3=${COMMIT} --url dreamteam3=${URL} ../dreamteam3/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the dreamteam3 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../dreamteam3/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/dreamteam3
SIGPATH=/some/root/path/dreamteam3-detached-sigs

./bin/gbuild --url dreamteam3=${BTCPATH},signature=${SIGPATH} ../dreamteam3/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dreamteam3-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dreamteam3-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dreamteam3-osx-build.assert
```
...
[dreamteam3/gitian.sigs](https://github.com/dreamteam3pay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
